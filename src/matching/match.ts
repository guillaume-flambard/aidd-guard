 ...
 *   4. Otherwise, Jaccard similarity on the criterion text.
 *
 * An AIDD criterion is one sentence, which is what makes similarity worth
 * computing at all here: there is no separate body to dilute the union.
 * `sharedTerms` is carried so any score can be justified line by line, because
 * a number nobody can explain is a number nobody should gate a build on.
 */
...
export function matchCriterion(
  ...
  const criterionText = normalize(criterion.text);
  ...
  for (const candidate of index.candidates) {
    ...
    const leafScore = jaccard(criterionText.set, candidate.leaf.set);
    const fullScore = jaccard(criterionText.set, candidate.full.set);
    const useLeaf = leafScore >= fullScore;
    const score = useLeaf ? leafScore : fullScore;
    if (score <= 0) continue;
    const terms = sharedTerms(criterionText, useLeaf ? candidate.leaf.set : candidate.full.set);
  ...
  if (best.score >= options.passThreshold && best.sharedTerms.length >= options.minSharedTerms) {
    // A skipped test is exactly the state aidd-guard exists to reveal, so it
